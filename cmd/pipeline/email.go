...
func (es *EmailSender) SendHeadlinesSummary(ctx context.Context, headlines []NotionHeadline) error {
	...
	// メール本文を生成
	body := es.generateEmailBody(filtered)
...
// generateEmailBody はAI Summary付きヘッドラインダイジェストのメール本文を生成する
//
// 【出力フォーマット】
//
//	炭素関連記事一覧 - 2026-01-06
//	合計: 25 記事
//
//	1. [AI Summaryの内容がここに表示される]
//	   https://carbonherald.com/...
//
//	2. [AI Summaryの内容がここに表示される]
//	   https://example.com/...
func (es *EmailSender) generateEmailBody(headlines []NotionHeadline) string {
	...
	for i, h := range headlines {
		// AISummaryがあればそれを使用、なければTitleをそのまま表示
		displayText := h.AISummary
		if displayText == "" {
			// フォールバック: タイトルをそのまま使用
			displayText = h.Title
		}
