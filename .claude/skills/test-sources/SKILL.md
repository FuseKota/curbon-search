...
| 引数パターン | `-sources` | `-perSource` | Notion挿入 |
|-------------|-----------|-------------|-----------|
| （なし） | `all-free` | `3` | なし |
| 数値のみ（例: `5`） | `all-free` | その数値 | なし |
| ソース名のみ（例: `carbonherald`） | そのソース名 | `5` | なし |
| ソース名 + 数値（例: `carbonherald 10`） | そのソース名 | その数値 | なし |
| カンマ区切り（例: `carbonherald,rmi`） | `carbonherald,rmi` | `5` | なし |
| `notion` を含む（例: `carbonherald notion`） | そのソース名 | `5` | あり |
| `notion` のみ | `all-free` | `3` | あり |
...
`notion` 引数が**ある**場合（`-notionClip` フラグを追加）：
