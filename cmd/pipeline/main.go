// main.go
// Carbon Relay パイプラインのエントリーポイント
//
// このプログラムは、カーボンニュース収集・分析・配信を自動化します。
// 2つの運用モードをサポート：
...
//      - 有料記事のヘッドラインから無料記事を検索
//      - OpenAI検索 + IDFマッチング
//      - Notion統合に最適
//
// 主要な処理フロー:
//   1. CLIフラグ解析（13フラグ）
//   2. .env から環境変数読み込み（OpenAI API Key, Notion等）
//   3. 見出し収集（18ソースから、または既存JSONから）
//   4. OpenAI Web検索（queriesPerHeadline > 0の場合）
//   5. IDFベースのマッチングとスコアリング
//   6. Notionクリッピング（-notionClip指定時）
//   7. メール送信（-sendEmail指定時）
package main
